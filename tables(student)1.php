...
{
	answer=confirm("Confirm delete all students?");
	return answer;
}
</script>
<body>
      ...
      <div class="templatemo-content-wrapper">
        <div class="templatemo-content">
          ...
          <div class="row margin-bottom-30">
			 ...
			 $result7=mysqli_query($conn,"select * from student,parent where student.parent_id_fk = parent.parent_id and parent.barred_status='barred';");
			...
			$result7_a=mysqli_query($conn,"select * from student,parent where student.parent_id_fk = parent.parent_id and parent.barred_status='barred' limit $page1,6;");
			
			if(isset($_POST["btnSearch"]))
           {
	        $kw=$_POST["search_box"];
	        $result7_a=mysqli_query($conn,"select * from student,parent where student.parent_id_fk = parent.parent_id and parent.barred_status='barred' and student.student_name like '%$kw%' 
			or student.parent_id_fk = parent.parent_id and parent.barred_status='barred' and student.student_ic like '%$kw%' 
			or student.parent_id_fk = parent.parent_id and parent.barred_status='barred' and student_subject like '%$kw%'");
           }
			?>
              <ul class="nav nav-pills">
				...
				<li class="active"><a href="tables(student).php?page=1">Student<span class="badge"><?php echo $row8?></span></a></li>
              </ul>          
            </div>
          </div> 
          <div class="row">
            <div class="col-md-12">
			 <div class="btn-group pull-right" id="templatemo_sort_btn">
                ...
                <h4 class="margin-bottom-15">List of Students <span style="color:red">(Barred)</span></h4>
				<ul>
			...
			?>
              ...
              <?php if($page >= 2){ ?>
                <li><a href="tables(student)1.php?page=<?php echo $previouspage;?>">&laquo;</a></li>
			  <?php } ?>
				<?php for($b=1;$b<=$a;$b++){?>
                <li><a href="tables(student)1.php?page=<?php echo $b?>"><?php echo $b?><span class="sr-only">(current)</span></a></li>
				<?php
				}
				?>
				<?php if($page != $endpage){ ?>
                <li><a href="tables(student)1.php?page=<?php echo $nextpage;?>">&raquo;</a></li>
				 <?php } ?>
			</ul>  
            </div>
          </div>
...
?>
<script type="text/javascript">
window.location.replace("tables(student)1.php?page=<?php echo $page?>"); 
</script>
<?php
}
?>
...
<?php
if(isset($_REQUEST["inactive_multiple1"])&&isset($_REQUEST["page"]))
{
	  $result_delete=mysqli_query($conn,"delete student from parent,student where student.parent_id_fk=parent_id and barred_status='barred'");
	  if($result_delete){
	  $row=mysqli_fetch_assoc(mysqli_query($conn,"select * from parent,student where student.parent_id_fk=parent_id and barred_status='barred'"));
	  $subject_venue=$row["schedule_id"];
	  $subject_venue=json_decode($subject_venue,true);
	  foreach($subject_venue as $subject => $id){
	  $result_venue=mysqli_query($conn,"select * from schedule where schedule_id='$id'");
