      ...
      <div class="templatemo-content-wrapper">
        <div class="templatemo-content">
          ...
          <div class="row margin-bottom-30">
			 ...
			 $result7=mysqli_query($conn,"select * from student,parent where student.parent_id_fk = parent.parent_id and student.activate=1 and student.isdeleted=0;");
			...
			$result7_a=mysqli_query($conn,"select * from student,parent where student.parent_id_fk = parent.parent_id and student.activate=1 and student.isdeleted=0
		    limit $page1,6;");
			
			if(isset($_POST["btnSearch"]))
           {
	        $kw=$_POST["search_box"];
	        $result7_a=mysqli_query($conn,"select * from student,parent where student.parent_id_fk = parent.parent_id and student.activate=1 and student.isdeleted=0 and student.student_name like '%$kw%' 
			or student.parent_id_fk = parent.parent_id and student.activate=1 and student.isdeleted=0 and student.student_ic like '%$kw%' 
			or student.parent_id_fk = parent.parent_id and student.activate=1 and student.isdeleted=0 and student_subject like '%$kw%'");
           }
			?>
          ...
          <div class="row">
			...
			<?php
			   ...
			   {
				   if($row['barred_status']="unbarred"&&$row['activate']=1&&$row['isdeleted']=0||$row['barred_status']="barred"||$row['barred_status']="blocked")
				   {
			 ?>
                  <tbody>
                    <tr>
			...
			?>
              ...
              <?php if($page >= 2){ ?>
                <li><a href="tables(student).php?page=<?php echo $previouspage;?>">&laquo;</a></li>
			  <?php } ?>
				<?php for($b=1;$b<=$a;$b++){?>
                <li><a href="tables(student).php?page=<?php echo $b?>"><?php echo $b?><span class="sr-only">(current)</span></a></li>
				<?php
				}
				?>
				<?php if($page != $endpage){ ?>
                <li><a href="tables(student).php?page=<?php echo $nextpage;?>">&raquo;</a></li>
				 <?php } ?>
			</ul>  
            </div>
          </div>
...
?>
<script type="text/javascript">
window.location.replace("tables(student).php?page=<?php echo $page?>"); 
</script>
<?php
}
?>
