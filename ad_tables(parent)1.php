      ...
      <div class="templatemo-content-wrapper">
        <div class="templatemo-content">
          ...
          <div class="row margin-bottom-30">
			 ...
			 $result8=mysqli_query($conn,"select * from parent where barred_status='barred';");
			...
			$result8_a=mysqli_query($conn,"select * from parent where barred_status='barred' limit $page1,6;");
			
			if(isset($_POST["btnSearch"]))
           {
	        $kw=$_POST["search_box"];
	        $result8_a=mysqli_query($conn,"select * from parent where barred_status='barred' and parent_name like '%$kw%' or barred_status='barred' and parent_ic like '%$kw%' or barred_status='barred' and parent_email like '%$kw%'");
           }
		   
           if(isset($_POST["barred_btn"]))
           {
			   mysqli_query($conn,"update parent set barred_status = 'blocked' where barred_status='barred'");
           ?>
           <script type="text/javascript">
           window.location.replace("ad_tables(parent)1.php?page=<?php echo $page?>"); 
           </script>
           <?php
            }
           ?>
              <ul class="nav nav-pills">
				...
				<li class="active"><a href="ad_tables(parent).php?page=1">Parent<span class="badge"><?php echo $row9?></span></a></li>
          ...
          <div class="row">
            <div class="col-md-12">
			 <div class="btn-group pull-right" id="templatemo_sort_btn">
                <button type="button" class="btn btn-default">Barred</button>
                <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
                  <span class="caret"></span>
                  <span class="sr-only">Toggle Dropdown</span>
                </button>
                <ul class="dropdown-menu" role="menu">
                  <li><a href="ad_tables(parent).php?page=1">Barred & Unbarred</a></li>
                  <li><a href="ad_tables(parent)1.php?page=1">Barred</a></li>
				  <li><a href="ad_tables(parent)3.php?page=1">Blocked</a></li>
                  <li><a href="ad_tables(parent)2.php?page=1">Unbarred</a></li>
                </ul>
              </div>
			  <h4 class="margin-bottom-15">List of Parents <span style="color:orange">(Barred)</span></h4>
			  <ul>
			 ...
			 ?>
                  <tbody>
                    <tr>
					  ...
					  <td><a href="ad_tables(parent)1.php?prid=<?php echo $row['parent_id']."&page=".$page;?>" onclick="return confirmation()" class="btn btn-danger">Blocked</a></td>
			...
			?>
			  ...
			  <div>
			  <input type="submit" class="btn btn-danger" name="barred_btn" value="Blocked All" onclick="return confirmation2()">
			  <input type="submit" class="btn btn-danger" name="inactive_multiple" value="Blocked Selected Records" onclick="return confirmation3()"/>
			  </div>
				</form>
              </div>
               <ul class="pagination pull-right">
              <?php if($page >= 2){ ?>
                <li><a href="ad_tables(parent)1.php?page=<?php echo $previouspage;?>">&laquo;</a></li>
			  <?php } ?>
				<?php for($b=1;$b<=$a;$b++){?>
                <li><a href="ad_tables(parent)1.php?page=<?php echo $b?>"><?php echo $b?><span class="sr-only">(current)</span></a></li>
				<?php
				}
				?>
				<?php if($page != $endpage){ ?>
                <li><a href="ad_tables(parent)1.php?page=<?php echo $nextpage;?>">&raquo;</a></li>
				 <?php } ?>
			</ul>  
            </div>
          </div>
...
if(isset($_REQUEST["inactive_multiple"])&&isset($_REQUEST["page"]))
{
	...
	{
	  $inac_id=$checkbox[$i];
      mysqli_query($conn,"update parent set barred_status='blocked' where parent_id=$inac_id");
	  mysqli_query($conn,"update student set parent_barred_status='blocked' where parent_id_fk=$inac_id");
	}	
?>
<script type="text/javascript">
window.location.replace("ad_tables(parent)1.php?page=<?php echo $page?>"); 
...
if(isset($_REQUEST["active_multiple"])&&isset($_REQUEST["page"]))
{
	...
	{
	  $ac_id=$checkbox[$i];
      mysqli_query($conn,"update parent set barred_status='barred' where parent_id=$ac_id");
	  mysqli_query($conn,"update student set parent_barred_status='barred' where parent_id_fk=$ac_id");
	}	
?>
<script type="text/javascript">
window.location.replace("ad_tables(parent)1.php?page=<?php echo $page?>"); 
...
if(isset($_REQUEST["prid"])&&isset($_REQUEST["page"]))
{
	$prid=$_REQUEST["prid"];
	$page=$_REQUEST["page"];
	mysqli_query($conn,"update parent set barred_status='blocked' where parent_id=$prid");
	mysqli_query($conn,"update student set parent_barred_status='blocked' where parent_id_fk=$prid");
?>
<script type="text/javascript">
window.location.replace("ad_tables(parent)1.php?page=<?php echo $page?>"); 
</script>
