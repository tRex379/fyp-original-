        ...
        <div class="templatemo-content">
         ...
         <h1>Payments</h1>
          ...
          <div class="row margin-bottom-30">
            <div class="col-md-12">
              <?php 
			 $result6=mysqli_query($conn,"select * from payment where payment_status='Yes';");
			 $row7=mysqli_num_rows($result6);
			 $result7=mysqli_query($conn,"select * from student where register_status='Yes';");
			...
			$result6_a=mysqli_query($conn,"select * from payment,parent where payment.parent_id_fk = parent.parent_id  and payment_status='Yes' limit $page1,6;");
			?>
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="table-responsive">
                 <h4 class="margin-bottom-15">Tuition Fees <span style="color:#ADFF2F;">(Accept)</span></h4>
              ...
              <?php if($page >= 2){ ?>
                <li><a href="tables(paymentTF1).php?page=<?php echo $previouspage;?>">&laquo;</a></li>
			  <?php } ?>
				<?php for($b=1;$b<=$a;$b++){?>
                <li><a href="tables(paymentTF1).php?page=<?php echo $b?>"><?php echo $b?><span class="sr-only">(current)</span></a></li>
				<?php
				}
				?>
				<?php if($page != $endpage){ ?>
                <li><a href="tables(paymentTF1).php?page=<?php echo $nextpage;?>">&raquo;</a></li>
				 <?php } ?>
			</ul>  
            </div>
          </div>
...
<?php
if(isset($_REQUEST["pid"])&&isset($_REQUEST["page"]))
{
  
	$pid=$_REQUEST["pid"];
	$page=$_REQUEST["page"];
	$type=$_POST["type"];
	mysqli_query($conn,"update payment set payment_status='Yes' where payment_id=$pid");
	
?>
<script type="text/javascript">
window.location.replace("tables(payment).php?page=<?php echo $page?>"); 
</script>
<?php
}
?>
...
{
	$pid1=$_REQUEST["pid1"];
	mysqli_query($conn,"update payment set payment_status='Reject' where payment_id=$pid1");
?>
<script type="text/javascript">
window.location.replace("tables(payment).php?page=<?php echo $page?>"); 
</script>
