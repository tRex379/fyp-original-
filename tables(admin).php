      ...
      <div class="templatemo-content-wrapper">
        <div class="templatemo-content">
          <ol class="breadcrumb">
            ...
            <li class="active">List of Admins</li>
          </ol>
         <h1>Management</h1>
		 <p>Updates records of the tuition center </p>
          <div class="row margin-bottom-30">
			...
			$result4_a=mysqli_query($conn,"select * from admin limit $page1,6");
			
			if(isset($_POST["btnSearch"]))
           {
	        $kw=$_POST["search_box"];
	        $result4_a=mysqli_query($conn,"select * from admin where Email like '%$kw%' or Contact_Number like '%$kw%'");
           }
			?>
              <ul class="nav nav-pills">
			    <li class="active"><a href="tables(admin).php?page=1">Admin<span class="badge"><?php echo $row5?></span></a></li>
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <h4 class="margin-bottom-15">List of Admins</h4>
              ...
              <input type="text" class="form-control" style="width:95%;display: inline-block;" id="search_box" name="search_box"  placeholder="Search by Email / Contact Number...">
			  ...
			  <?php
			   while($row=mysqli_fetch_assoc($result4_a))
			   {
			  ?>
                  <tbody>
                    <tr>
                      ...
                      <td><a href="edit(admin).php?adid=<?php echo $row['id']."&page=".$page;?>" class="btn btn-info">Update</a></td> 
                      <td>
					  <?php if($row["Status"]=="active"){
						  ?>
						  <a href="tables(admin).php?adid=<?php echo $row['id']."&page=".$page;?>" onclick="return confirmation()" class="btn btn-danger">Inactive</a>
						  <?php
					      }
						  else if(($row["Status"]=="inactive"))
						  {
						  ?>
						   <a href="tables(admin).php?adid1=<?php echo $row['id']."&page=".$page;?>" onclick="return confirmation1()" class="btn btn-success">Active</a>
						  <?php
						  }
						  ?>
					  </td>
			  ...
			  <?php if($page >= 2){ ?>
                <li><a href="tables(admin).php?page=<?php echo $previouspage;?>">&laquo;</a></li>
			  <?php } ?>
				<?php for($b=1;$b<=$a;$b++){?>
                <li><a href="tables(admin).php?page=<?php echo $b?>"><?php echo $b?><span class="sr-only">(current)</span></a></li>
				<?php
				}
				?>
				<?php if($page != $endpage){ ?>
                <li><a href="tables(admin).php?page=<?php echo $nextpage;?>">&raquo;</a></li>
				 <?php } ?>
              </ul>  
            </div>
          </div>
...
<?php
if(isset($_REQUEST["adid"])&&isset($_REQUEST["page"]))
{
	$adid=$_REQUEST["adid"];
	$page=$_REQUEST["page"];
	mysqli_query($conn,"update admin set Status='inactive' where id=$adid");
?>
<script type="text/javascript">
window.location.replace("tables(admin).php?page=<?php echo $page?>"); 
</script>
<?php
}
?>
...
{
	$adid1=$_REQUEST["adid1"];
	mysqli_query($conn,"update admin set Status='active' where id=$adid1");
?>
<script type="text/javascript">
window.location.replace("tables(admin).php?page=<?php echo $page?>"); 
</script>
...
if(isset($_REQUEST["inactive_multiple"])&&isset($_REQUEST["page"]))
{
	...
	{
	  $inac_id=$checkbox[$i];
      mysqli_query($conn,"update admin set Status='inactive' where id=$inac_id");
	}	
?>
<script type="text/javascript">
window.location.replace("tables(admin).php?page=<?php echo $page?>"); 
...
if(isset($_REQUEST["active_multiple"])&&isset($_REQUEST["page"]))
{
	...
	{
	  $ac_id=$checkbox[$i];
      mysqli_query($conn,"update admin set Status='active' where id=$ac_id");
	}	
?>
<script type="text/javascript">
window.location.replace("tables(admin).php?page=<?php echo $page?>"); 
